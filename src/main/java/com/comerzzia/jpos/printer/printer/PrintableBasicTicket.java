...
import com.comerzzia.jpos.printer.ticket.PrintItem;
...
public class PrintableBasicTicket implements Printable {
	...
	public PrintableBasicTicket(BasicTicket ticket, int imageable_x,
			int imageable_y, int imageable_width, int imageable_height) {
		this.ticket = ticket;
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
		...
		while (line < commands.size()) {
			...
			if (currentpage < pageIndex) {
				...
				printed = true;
				commands.get(line).draw(g2d, 0, currentpagey - itemheight,
						imageable_width);
