...
package com.comerzzia.jpos.printer.printer;
...
import com.comerzzia.jpos.printer.ticket.BasicTicketForPrinter;
import com.comerzzia.jpos.util.ReportUtils;
import com.comerzzia.jpos.util.SelectPrinter;
import es.mpsistemas.util.log.Logger;


public class DevicePrinterPrinter implements DevicePrinter {

	private static final Logger log = Logger.getMLogger(DevicePrinterPrinter.class);
	 ...
	 */
	public DevicePrinterPrinter(String printername,//Se ha quitado el component parent.
		...
		try {
			...
			if (printservice == null) {
				...
				if (printers.length == 0) {
					log.warn("No hay impresoras");
					ps = null;
				} else {
					SelectPrinter selectprinter = SelectPrinter
							.getSelectPrinter(parent, printers);
					selectprinter.setVisible(true);
					if (selectprinter.isOK()) {
						ps = ReportUtils.getPrintService(selectprinter
								.getPrintService());
					} else {
						ps = null;
					}
			...
			if (ps != null) {
				...
				Doc doc = new SimpleDoc(new PrintableBasicTicket(
						m_ticketcurrent, imageable_x, imageable_y,
			...
			log.error("ERROR DE IMPRESORA", ex);
