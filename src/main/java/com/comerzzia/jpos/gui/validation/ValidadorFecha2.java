 ...
 * @author MGRI
 */
public class ValidadorFecha2 extends ValidadorForm {
    ...
    private static SimpleDateFormat formatoDeFecha5 = new SimpleDateFormat("dd/MM/yyyy", new Locale("es","ES"));
    Date fechaInicio;
    Date fechaFin;
    private static Logger log = Logger.getMLogger(ValidadorEntero.class);

    public ValidadorFecha2() {
        this.fechaFin = null;
        this.fechaInicio = new Date(0);
    }

    public ValidadorFecha2(Date fechaFin) {
        this.fechaFin = fechaFin;
        this.fechaInicio = new Date(0);
    }

    public ValidadorFecha2(Date fechaInicio, Date fechaFin) {
                            ...
                            try {                                
                                ...
                                try {
                                    ...
                                    if (fechaActualTexto.length()==8){           
                                      ...
                                      try {
                                    fechaActual = formatoDeFecha5.parse(fechaActualTexto);                                    
                                    //formato ddMMyyyy
                                    
                                    if (fechaActualTexto.length()==10){           
                                    
                                        diaI = Integer.parseInt(fechaActualTexto.substring(0, 2));
                                        if (diaI >31 || diaI<1 ){
                                            throw new ValidationFormException("Ha introducido un día invalido para el formato dd/MM/yyyy");
                                        } 
                                        mesI = Integer.parseInt(fechaActualTexto.substring(3, 5));
                                        if (mesI >12 || mesI<1 ){
                                            throw new ValidationFormException("Ha introducido un mes invalido para el formato dd/MM/yyyy");
                                        } 
                                        validaFechaEntre(fechaActual,(IValidableForm)comp);
                                       ...
                                       throw new ValidationFormException("La fecha introducida no es válida, ha de tener formato dd/MM/yyyy");
                                    }
                                }
    ...
    private void validaFechaEntre(Date fechaActual,IValidableForm comp) throws ValidationFormException {
        if (comp instanceof IValidableForm){
            comp.setText(formatoDeFecha5.format(fechaActual));
        }
