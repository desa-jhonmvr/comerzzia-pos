 ...
 * @author SMLM
 */
public class JAnulacionDocumentoConAbono extends JVentanaDialogo implements IViewerValidationFormError {

    private static Logger log = Logger.getMLogger(JAnulacionDocumentoConAbono.class);
    List<IValidableForm> formulario;
    String tipo = null;

    /** Creates new form JReimpresionFactura */
    public JAnulacionDocumentoConAbono() {
        super();
    ...
    /** Creates new form JReimpresionFactura */
    public JAnulacionDocumentoConAbono(String tipo) {
        super();
        this.tipo = tipo;
        try{
            formulario = new LinkedList<IValidableForm>();
            ...
            if(tipo.equals(AnulacionesServices.ABONO_RESERVA)) {
                l_documento.setText("Abono de Reserva: ");
                jLabel6.setText("ANULACIÓN ABONO A RESERVACIÓN");
            }       
            if(tipo.equals(AnulacionesServices.ABONO_PLAN_NOVIO)) {
                l_documento.setText("Abono de Plan Novio: ");
                jLabel6.setText("ANULACIÓN ABONO A PLAN NOVIO");
            }
    ...
    private void initComponents() {
        ...
        t_factura4.setNextFocusableComponent(jTextArea);
    ...
    public void accionAceptar() {
        try {
            ...
            if(tipo.equals(AnulacionesServices.ABONO_RESERVA)){
                 ...
                 */
                AnulacionesServices.anularAbonoReservacion(t_factura1.getText(), t_factura3.getText().trim(), t_factura4.getText().trim(),jTextArea.getText());
                JPrincipal.getInstance().crearConfirmacion("El abono de reserva se ha anulado correctamente");              
            }
            if(tipo.equals(AnulacionesServices.ABONO_PLAN_NOVIO)){
                /**
                 *Rd Se agrega el campo Observacion
                 */
                AnulacionesServices.anularAbonoPlanNovio(t_factura1.getText(), t_factura3.getText().trim(), t_factura4.getText().trim(),jTextArea.getText());
                JPrincipal.getInstance().crearConfirmacion("El abono de plan novio se ha anulado correctamente");                       
            }
        ...
        try {
            ...
            if(documentoBean != null){
                if(tipo.equals(DocumentosBean.ABONO_RESERVA)){
                    PrintServices.getInstance().reimpresionComprobanteAbono(documentoBean,true,false);
                }    
                if(tipo.equals(DocumentosBean.ABONO_PLAN_NOVIO)){
                    PrintServices.getInstance().reimpresionComprobanteAbono(documentoBean,true,false);
                }                  
            } else {
                JPrincipal.getInstance().crearAdvertencia("No se han encontrado abonos para los datos introducidos.");
            }        
        } catch (ValidationFormException ex) {
            addError(ex);
