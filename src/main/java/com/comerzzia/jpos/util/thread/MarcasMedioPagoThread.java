 ...
 */
public class MarcasMedioPagoThread extends Thread {

    private static final Logger LOG_POS = Logger.getMLogger(MarcasMedioPagoThread.class);

    private static final int SOCKET = 15290;
    ...
    public void run() {
        ...
        try {
            Socket s = null;
            try {
                ss = new ServerSocket(Constantes.SOCKET_MARCAS_MEDIO_PAGO);
                s = ss.accept();
            ...
            try {
                ...
                List<MarcaTarjetaOnlineDTO> marcas = formaPagoServices.obtenerMarcasTarjetasMedioPago();

                responseDTO.setExito(Boolean.TRUE);
                responseDTO.setObjetoRespuesta(marcas);
            } catch (MedioPagoException ex) {
                String error = "Error en la marca de la tarjeta " + ex.getMessage();
